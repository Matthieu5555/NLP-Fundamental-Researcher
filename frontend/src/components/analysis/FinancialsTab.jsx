        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-100">
              ...
              {headers.map((h, idx) => (
                <th key={idx} className="px-3 py-2 text-right font-semibold text-slate-700">{h}</th>
              ))}
            </tr>
          </thead>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">5-year financial data with key ratios and highlights</p>
        </div>
        {companyInfo && (
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${companyInfo.is_us ? 'bg-accent-success-light text-accent-success-dark' : 'bg-slate-100 text-slate-600'}`}>
      ...
      {financialsLoading ? (
        <div className="text-center py-12">
          <div className="inline-flex items-center justify-center space-x-3 mb-4">
            <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--brand-primary)', borderTopColor: 'transparent' }}></div>
          </div>
          ...
          {financialStatements.income_statements?.length > 0 && (() => {
            ...
            return (
              <StatementTable title="Income Statement" dotColor="bg-accent-info" headers={getPeriodHeaders(stmts)}>
                ...
                <StatRow label="Gross Profit" data={stmts} accessor={s => formatBillions(s.gross_profit)} className="bg-slate-50/50" />
                ...
                <StatRow label="Gross Margin" data={stmts} accessor={s => formatPercent(s.gross_margin)} className="bg-accent-info-light/50 font-medium" />
                <StatRow label="Net Margin" data={stmts} accessor={s => formatPercent(s.net_margin)} className="bg-accent-info-light/50 font-medium" />
              </StatementTable>
            )
          })()}
          ...
          {financialStatements.balance_sheets?.length > 0 && (() => {
            ...
            return (
              <StatementTable title="Balance Sheet" dotColor="bg-accent-success" headers={getPeriodHeaders(stmts)}>
                ...
                <StatRow label="Total Equity" data={stmts} accessor={s => formatBillions(s.total_equity)} className="bg-slate-50/50" />
                <StatRow label="Cash" data={stmts} accessor={s => formatBillions(s.cash)} />
                <StatRow label="Total Debt" data={stmts} accessor={s => formatBillions(s.total_debt)} className="bg-slate-50/50" />
                <StatRow label="Debt/Equity" data={stmts} accessor={s => s.debt_to_equity ? `${s.debt_to_equity.toFixed(2)}x` : 'N/A'} className="bg-accent-success-light/50 font-medium" />
              </StatementTable>
            )
          })()}
          ...
          {financialStatements.cash_flows?.length > 0 && (() => {
            ...
            return (
              <StatementTable title="Cash Flow Statement" dotColor="bg-chart-sma50" headers={getPeriodHeaders(stmts)}>
                ...
                <StatRow label="Free Cash Flow" data={stmts} accessor={s => formatBillions(s.free_cash_flow)} className="bg-slate-50/50" />
                <StatRow label="CapEx" data={stmts} accessor={s => formatBillions(s.capex)} />
                <StatRow label="FCF Margin" data={stmts} accessor={s => formatPercent(s.fcf_margin)} className="bg-chart-sma50/10 font-medium" />
              </StatementTable>
            )
          })()}
          ...
          {financialStatements.ratios?.length > 0 && (() => {
            ...
            return (
              <StatementTable title="Key Ratios" dotColor="bg-brand" headers={getPeriodHeaders(ratios)}>
                ...
                <StatRow label="ROA" data={ratios} accessor={r => formatPercent(r.profitability?.roa)} className="bg-slate-50/50" />
