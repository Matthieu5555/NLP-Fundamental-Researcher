function ScenarioAnalysis({ scenarioData }) {
  if (!scenarioData) return null

  const { bull, base, bear, weighted_fair_value, current_price, weighted_upside_pct } = scenarioData
    ...
    <div className="space-y-6">
      ...
      <div className={`rounded-xl p-6 border-2 ${isUndervalued ? 'bg-accent-success-light border-accent-success' : 'bg-accent-danger-light border-accent-danger'}`}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-600">Probability-Weighted Fair Value</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {scenarios.map((scenario) => {
          ...
          return (
            <div key={scenario.name} className={`rounded-lg p-4 border-2 ${c.bg} ${c.border}`}>
              {/* Header */}
              ...
              {scenario.revenue_yr5 > 0 && (
                <div className="mb-3">
                  ...
                  <p className="text-sm font-semibold text-slate-800">{fmtB(scenario.revenue_yr5)}</p>
                </div>
              )}
              ...
              {scenario.assumptions && (
                <div className="space-y-1 pt-2 border-t border-slate-200/60">
                  ...
                  {scenario.assumptions.wacc != null && (
                    <p className="text-xs text-slate-600">WACC: {fmtPct(scenario.assumptions.wacc)}</p>
                  )}
                  {scenario.assumptions.terminal_growth_rate != null && (
                    <p className="text-xs text-slate-600">TGR: {fmtPct(scenario.assumptions.terminal_growth_rate)}</p>
                  )}
                </div>
              )}
            </div>
