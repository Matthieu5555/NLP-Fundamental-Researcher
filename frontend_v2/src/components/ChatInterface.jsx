  ...
  return (
    <div className="bg-surface-elevated rounded-lg border overflow-hidden">
      ...
      <div className="border-b bg-surface-secondary px-6 py-5">
        <div>
          <h2 className="text-2xl font-bold text-primary">Dive Deeper on {ticker}</h2>
      ...
      {/* Chat Messages */}
      <div className="h-[500px] overflow-y-auto px-6 py-6 bg-surface-elevated space-y-4">
        {messages.length === 0 && !streaming && (
          <div className="text-center py-12">
            <div className="space-y-2 max-w-2xl mx-auto">
              <p className="text-xs text-tertiary font-semibold mb-3">Example questions:</p>
              {exampleQuestions.map((question, idx) => (
                <button
                  key={idx}
                  onClick={() => setInput(question)}
                  className="block w-full text-left px-4 py-3 text-sm text-brand bg-surface-elevated rounded-lg border hover:bg-brand-surface transition-colors"
                >
                  {question}
                </button>
              ))}
        ...
        {messages.map((message, idx) => (
          ...
          >
            ...
            {message.role === 'assistant' && message.acknowledgment && (
              <div className="mb-2">
                <div className="text-sm px-4 py-2 rounded-lg border bg-brand-surface text-brand-dark flex items-center gap-2">
            ...
            <div className={`max-w-[85%] ${message.role === 'assistant' ? 'space-y-2' : ''}`}>
              <div
                className={`rounded-2xl px-5 py-3 ${
                  message.role === 'user'
                    ? 'text-white bg-brand'
                    : 'bg-surface-elevated border text-primary'
                }`}
              >
                {message.role === 'user' ? (
                      ...
                      <div key={srcIdx} className="flex items-start gap-2">
                        ...
                        >
                          ...
                          {source.date && source.date !== 'recent' && (
                            <span className="text-tertiary ml-1">({source.date})</span>
                          )}
                        </a>
                      </div>
                    ))}
        ...
        {streaming && acknowledgment && (
          <div className="flex justify-start mb-2">
            <div className="text-sm px-4 py-2 rounded-lg border border-brand-border bg-brand-surface text-brand-dark flex items-center gap-2">
              <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="font-medium">
        ...
        {streaming && streamingMessage && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-2xl px-5 py-3 bg-surface-elevated border text-primary">
              <div className="prose prose-sm prose max-w-none">
        ...
        {streaming && !streamingMessage && (
          <div className="flex justify-start">
            <div className="rounded-2xl px-5 py-3 bg-surface-elevated border">
              {searchStatus ? (
                ...
                <div className="flex space-x-2">
                  ...
                  <div className="w-2 h-2 rounded-full bg-brand animate-bounce" style={{ animationDelay: '0.1s' }}></div>
                  <div className="w-2 h-2 rounded-full bg-brand animate-bounce" style={{ animationDelay: '0.2s' }}></div>
                </div>
              )}
            </div>
          </div>
      ...
      {beliefNotice && (
        <div className="px-6 py-3 border-t bg-brand-surface">
      ...
      {/* Input Form */}
      <div className="border-t bg-surface-elevated px-6 py-5">
        <form onSubmit={sendMessage} className="flex space-x-3">
          <textarea
            ...
            disabled={streaming}
            rows={2}
            className="flex-1 px-4 py-3 border border-medium rounded-lg focus:ring-2 focus:ring-brand focus:border-brand resize-none disabled:bg-surface-tertiary disabled:cursor-not-allowed transition-colors text-sm"
          />
          <button
            type="submit"
            disabled={!input.trim() || streaming}
            className="px-6 py-3 text-white font-semibold rounded-lg disabled:bg-surface-tertiary disabled:text-muted disabled:cursor-not-allowed transition-colors bg-brand hover:bg-brand-dark"
          >
            {streaming ? (
        ...
        <p className="text-xs text-tertiary mt-3 text-center">
          Press <kbd className="px-2 py-1 bg-surface-card-hover rounded text-xs font-mono">Enter</kbd> to send • <kbd className="px-2 py-1 bg-surface-card-hover rounded text-xs font-mono">Shift + Enter</kbd> for new line
        </p>
      </div>
    </div>
  )
